...
Vehicle::Vehicle(int lane, int s, int v, int a) {
    ...
    state = "CS";
    max_acceleration = -1;
    availableStates["CS"] = {"KL"};
    availableStates["KL"] = {"KL","PLCL", "PLCR"};
...
vector<Vehicle::SnapShot> Vehicle::generate_trajectory(string state,map<int, vector < vector<int> > >  predictions,int horizon){
